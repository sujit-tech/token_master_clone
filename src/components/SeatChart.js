...
const SeatChart = ({ occasion, tokenMaster, provider, setToggle }) => {
  ...
  const getSeatsTaken = async () => {
    const seatTaken = await tokenMaster.getSeatTaken(occasion.id);
    setSeatTaken(seatTaken);
  };
    ...
    <div className="occasion">
      <div className="occasion__seating">
        ...
        <div className="occasion__spacer--1">
          <strong>WALKTROUGH</strong>
        </div>
        {seatTaken &&
          Array(Number(occasion.maxTicket) - 50)
            .fill(1)
            .map((e, i) => (
              <Seat
                i={i}
        ...
        {seatTaken &&
          Array(25)
            ...
            .map((e, i) => (
              <Seat
                i={i}
                step={Number(occasion.maxTicket) - 24}
                columnStart={22}
                maxColumns={5}
                rowStart={2}
                maxRows={5}
